...
- **Deep-clone the rendered DOM on cache set / cache get** (the model before Phase 12): simple isolation between widget instances, but `cloneNode(true)` doesn't copy `<canvas>` pixel buffers or `<iframe>`/`<video>` playback state, which forced an exclusion branch that re-executed on every render for those containers. Rejected in Phase 12 in favor of the live-reference cache.
