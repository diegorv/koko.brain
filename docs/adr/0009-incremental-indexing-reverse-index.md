...
Index dedupe (`src/lib/utils/index-dedupe.ts`): shared `Map<path, lastContent>` with `isAlreadyIndexed` / `markIndexed` / `clearIndexedEntry` / `clearAllIndexed`. Both `updateIndexesForFile` (content-effect) and `notifyAfterSave` guard on this signature up front; delete on `removeFileFromIndex` so a re-created file with identical bytes re-indexes; `resetHooks` wipes on vault teardown. Memory cost is a Map entry per file — the content string is a reference shared with `noteContents`, not a copy.
...
- Any function that mutates vault state (delete, rename, move) must call `clearIndexedEntry(path)` or risk stale dedupe skipping a re-creation. `removeFileFromIndex` does this; new code that deletes files must too.
