...
- Any function that mutates vault state (delete, rename, move) must route through the owner's delete branch or risk a stale dedupe signature skipping a re-creation, and a deleted note lingering as a phantom page in every per-file index. Amendment 2026-08-19 (issue 39): the `forgetNote(path)` adapter now lives in `path-change.service.ts`, not in `fs.service.ts`, and it is no longer what the file explorer reaches for. Anything that changes a note's PATH (delete, rename, move) calls `applyPathChange({ from, to, isDirectory, diskOp })`, which owns the ordering, walks a renamed folder's children out of `fsStore.fileTree` BEFORE the disk op (a directory key is a member of no per-file index, so the exact-key removal alone reaches nothing), and invokes the Rust `rename_note` re-key BEFORE the per-path sweep. `forgetNote` stays the adapter for a single note vanishing on its own; the watcher calls `applyNoteChange` directly with a `vaultPath` so the FTS5 row goes too. New code that removes a note from a path must use one of the three - never a hand-rolled `clearIndexedEntry` + `remove_note_from_index` pair.
