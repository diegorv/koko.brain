...
8. **QueryJS widgets track every `kb.view(…)` / `dv.view(…)` promise so bare calls complete before caching** — `kb-api.ts` pushes each `view()` promise into `_pendingViews` and exposes `awaitAllPending()`; `queryjs-block-widget.ts → execute()` awaits all pending after the user code returns, then stores the rendered container in `queryjsSessionStore`. Without this, a block like `` ```queryjs\nkb.view("…")\n``` `` (no explicit `await`) would return `undefined` synchronously and the cache would capture an EMPTY container before the inner script finishes writing DOM. The legacy regex that prepended `await` to top-level `kb.view(` / `dv.view(` calls was retired in Phase 12 of the Híbrido D refactor in favor of this tracker — smaller, covers aliased calls, lives alongside the data it tracks.

9. **`queryjsSessionStore` stores LIVE DOM references by content hash — not clones** — the widget cache (`queryjs-session.store.svelte.ts`) keeps `Map<contentHash, HTMLElement>` of the rendered DOM itself; on cache hit the widget moves the same element into the fresh CodeMirror container, preserving `<canvas>` pixel buffers, `<iframe>`/`<video>` playback state and any other mutable DOM state for free. The legacy `scriptResultCache` deep-cloned the container via `cloneNode(true)` and excluded containers with those element types because `cloneNode` doesn't copy their state; both the clone and the exclusion branch were retired in Phase 12. Invalidation stays path-scoped: `editor.service.ts → closeTab` calls `queryjsSessionStore.invalidatePath(path)`, and `invalidateQueryjsCache()` (called on save / vault rebuild) is a shim around `queryjsSessionStore.reset()`.

10. **QueryJS execution policy is settings-driven** — `settings.queryjs.autoRunQueries: 'first-open' | 'always' | 'manual'` picks what happens on cache miss. `'first-open'` (default) runs once per note per session and shows `▶ Run` on subsequent misses (typical: the block was edited, hash changed). `'always'` matches legacy behavior (re-run every miss). `'manual'` always shows `▶ Run`. Policy is read on every `toDOM()`; the session store's `hasAutoRun(path)` / `markAutoRun(path)` back the first-open semantics. Never flip `markAutoRun` in `'manual'` mode — doing so would make a later switch to `'first-open'` skip the first-open behavior.
