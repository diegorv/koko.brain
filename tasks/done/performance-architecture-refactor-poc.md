# Performance & Architecture Refactor (ABANDONED PoC)

> **Status: ABANDONED PoC.** Despite `[x]` checkboxes in Phases 0, 1, and 2 below, **none** of the artifacts they describe were actually implemented. Codebase audit on 2026-04-28 confirmed: no `src-tauri/src/vault/` or `index/` modules, no `docs/perf/` directory, no `docs/adr/0025-...`, no `scripts/perf-baseline.py`, no `notify` crate in `Cargo.toml`, no `experimental` settings group, no `vaultIndexVersion` signal. The architectural design here is sound and was carried forward; only the false `[x]` history is being archived. **Superseded by `tasks/todo/performance-architecture-refactor.md` (rewritten 2026-04-28).**
