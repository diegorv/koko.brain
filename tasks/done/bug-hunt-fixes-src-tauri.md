...
- [x] #8 block-array stray `- ` (dash + only whitespace) item -> Rust continues, TS breaks. `vault/parsing.rs:323`. Impact LOW. Complexity LOW (~2 LOC). Fix (if pursued): `break` when `after_dash.is_empty()`.
