...
- [x] Task 4.1 (P1): Add `createFolder(path)` wrapper to `fs-rust.service.ts` and retrofit every raw `invoke('create_folder', { path })` call site (note-creator, trash, fs.service, deep-link, templates, bookmarks) to use it. Discovered mid-Task-13: each consumer had been bolting `invoke('create_folder', ...)` directly into the source.
- [x] Task 4.2 (P1): Add `path_exists_raw(path)` Rust command + `pathExistsRaw(path)` TS wrapper that deliberately skip `validate_vault_path`. Required because the wrapper signature `pathExists(vaultPath, path)` canonicalizes the vault root, which fails when the vault directory has been deleted - the exact stale-recent-vault scenario Task 22 needs to detect.
...
- [x] Task 14 (P3): Migrate `src/lib/features/file-icons/file-icons.service.ts` -> wrappers. Update tests.
...
- [x] Task 22 (P5): Migrate `src/lib/core/vault/vault.service.ts` (exists -> `pathExistsRaw`). Closes the stale-recent-vault detection bug. Uses the vault-less wrapper from Task 4.2 because `pathExists(vaultPath, path)` canonicalizes the vault root, which fails when the vault has been deleted - the exact case being detected. Smoke: delete a recent vault folder externally, click recent -> toast appears, entry removed. Update `src/tests/lib/core/vault/`.
- [x] Task 23 (P5): Decide and migrate `src/lib/core/settings/settings.service.ts`. Resolved: settings file lives at `${vaultPath}/.kokobrain/settings.json` (vault-scoped) - migrated to wrappers. Update tests.
- [x] Task 24 (P5): Cleanup pass. Migrated the last hidden plugin-fs callsite (`live-preview/widgets.ts::loadEmbedContent` - dynamic `import('@tauri-apps/plugin-fs')` was invisible to earlier greps). Narrowed `src-tauri/capabilities/default.json` to `$APPLOG`-only `fs:allow-write-text-file`, `fs:allow-mkdir`, `fs:allow-exists` and dropped `fs:allow-read-dir`, `fs:allow-read-text-file`, `fs:allow-remove`, `fs:allow-rename`, `fs:allow-copy-file`, `fs:allow-watch`, `fs:allow-unwatch` entirely (no vault-path consumer remains). Deleted the now-orphaned `src/tests/fixtures/tauri-fs.fixture.ts` (zero importers). Kept `@tauri-apps/plugin-fs` in `package.json` + the Rust plugin registration because `log.service.ts` still uses it for `$APPLOG`. The manual smoke test (move vault to `$HOME/Documents/Brain`, exercise create/rename/move/delete + canvas + kanban + deep-link + bookmarks/file-icons/auto-move) is left to the user before tagging the next release.
