...
- [ ] Task 2: Rust — add `#[tauri::command] get_semantic_file_status(file_path: String) -> SemanticFileStatus` in `src-tauri/src/commands/semantic.rs`. Returns `{ chunkCount: u64, lastEmbeddedAt: Option<i64>, modelLoaded: bool }`. `file_path` is the vault-relative path (same convention as `update_semantic_file`). Register in `src-tauri/src/lib.rs` invoke_handler. Add a struct in `semantic/types.rs` (`SemanticFileStatus`, camelCase serde rename). Smoke-test through cargo test.
