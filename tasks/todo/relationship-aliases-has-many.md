...
- [ ] Task 3: TS — `vault-v2.types.ts` add `hasMany`; `PropertiesView.svelte` FIXED_RELATIONSHIPS keys `_belongs_to`/`_related_to`/`_has_many` + labels; `portent-filters.test.ts` keys `_belongs_to`/`_related_to`; `properties.logic.test.ts` `formatRelationshipLabel('has_many')`. (Frontend only -> pnpm check + pnpm vitest run.)
