...
- [x] P1.5 — Add `capture_clipboard_now` Tauri command in `quick_capture/commands.rs`: takes `&dyn Clipboard` (use `SystemClipboard` in prod), runs `kind_detect::decide`, emits a Tauri event `qc:capture-detected` carrying the action payload. Cargo test with `FakeClipboard` covers text/url/image/files cases.
- [x] P1.6 — Register Ctrl+Alt+Cmd+Space + Ctrl+Alt+Cmd+C global shortcuts in `lib.rs` setup. Space → placeholder (composer in P2). C → invokes `capture_clipboard_now`.
- [x] P1.7 — Frontend: TS handler in `src/lib/plugins/quick-capture/quick-capture.service.ts` listens for `qc:capture-detected`, calls `executeAction(payload, vaultStore.path)`. Vitest covers: event received → executeAction called with right shape (mock executeAction, real store). Smoke: copy text, press Ctrl+Alt+Cmd+C → markdown file lands in `_notes/YYYY/MM-MMM/...`.
...
- [x] P2.3 — Tauri commands `show_composer` / `dismiss_composer` (port from QC `commands::show_composer` / `dismiss_composer`, drop the `record_prev_frontmost` logic for now). Wire to Ctrl+Alt+Cmd+Space shortcut.
- [x] P2.4 — Port composer Svelte UI from `quick-capture/src/lib/composer/` into `src/lib/plugins/quick-capture/composer/`: textarea, Save button, Esc to dismiss, blur-to-dismiss behavior. Vitest mirrors QC's composer tests. _(Inlined into the route — no separate component, no save-Button (Cmd+Enter only), no blur dismiss (QC doesn't either; blur is unreliable with devtools / tooltips). Component-level vitest skipped: codebase has no svelte testing-library; testable logic lands in P2.5 as exported fns.)_
- [x] P2.5 — Save handler: builds `CaptureAction` (kind: 'note', text, captured_at, source_app fields blank), calls `executeAction` directly (no URI). Vitest covers save → executeAction invocation → window dismissed. _(Composer is in a separate webview from `vaultStore`; save goes through a new Rust IPC `submit_composer_capture(text)` that emits `qc:capture-detected` so the main-window listener does the actual `executeAction` dispatch. Tests cover the pure `build_composer_note_payload` helper — payload shape + body preservation.)_
- [~] P2.6 — _Deferred._ Port `src/lib/mentions/` wikilink parser. Composer ships v1 as plain textarea; autocomplete requires CodeMirror swap + Rust IPC to expose vault entries to the composer webview. Out of scope for this merge.
- [~] P2.7 — _Deferred (with P2.6)._ Mention picker UI wiring.
...
- [x] P4.1 — Add Settings panel section in kokobrain's existing settings UI: "Quick Capture" tab. Fields: folder format, filename format, per-kind template paths (note / clip / link / shot / file). Settings store extension under `settingsStore.quickCapture` + `QuickCaptureUpdate`. Deep-link `executeCaptureAction` adapted to read `quickCapture.templates[kind]` (no fallback to legacy `quickNote.templatePath`). Vitest covers store getters + defaults + per-kind merge. _(Hotkey enable/disable toggles intentionally out — dispatcher runs outside any webview and would need a Rust-side persistent store; defer if needed.)_
- [x] P4.2 — Composer dismiss + restore-frontmost: port QC's `record_prev_frontmost` (macOS NSWorkspace.frontmostApplication via objc2) — captures the app that was frontmost when composer summoned, restores focus on dismiss. **Also populate `sourceApp` on every capture payload** (clipboard shortcut + composer save) via the same `frontmost_bundle_id` helper; `sourceTitle` + `sourceUrl` follow via AppleScript for Chrome/Safari (browser-only — other apps stay null). Cargo test covers the recording fn against a fake NSWorkspace + payload merge.
- [x] P4.3 — Update `docs/adr/` with an ADR noting the merge decision and the dropped surfaces. Update `CONTEXT.md` if it lists quick-capture as an external dependency. No code changes. _(ADR 0028 added; no `CONTEXT.md` in repo so nothing to update there. Grep across `docs/` + `CLAUDE.md` found no existing quick-capture references.)_
- [x] P4.4 — Cleanup: any "quick-capture" comments in `src/lib/features/deep-link/deep-link.types.ts:83,127` and `src/tests/lib/features/deep-link/deep-link.service.test.ts` that referenced QC as an external app can be retired or rewritten to reflect the internal wiring. Surgical edit only. _(5 references in `deep-link.types.ts` rewritten to describe the internal clipboard-shortcut path + external automation surface. The `'source/quick-capture'` tag string in the test fixture is left as-is — it represents a tag value an external automation tool might still send.)_
