...
- [ ] P1.2 — Create `src-tauri/src/quick_capture/` module skeleton (mod.rs + clipboard.rs + kind_detect.rs + shortcuts.rs + commands.rs). Wire into `lib.rs`. Empty fns + module declarations.
- [ ] P1.3 — Port `clipboard.rs` (Clipboard trait, SystemClipboard, FakeClipboard, ClipboardSnapshot enum, ClipboardError, image-to-png helper) **verbatim** from QC `src-tauri/src/clipboard/mod.rs`. Port cargo tests verbatim.
- [ ] P1.4 — Port `kind_detect.rs` **adapting types**: instead of QC's `CaptureInput`/`ShotSource`, emit a struct that mirrors kokobrain's `CaptureAction` shape from `src/lib/features/deep-link/deep-link.types.ts` (kind + text/url/path + optional title/tags/sourceApp/etc.). Keep `decide()` pure. Port all URL-detection and file-mime tests.
