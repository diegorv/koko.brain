...
- [x] **8.7** Migrated `note-creator.service.ts::openOrCreateNote`: when `experimental.rustProperties` is on, replaces `mkdir` + `writeTextFile` with `invoke('create_note', { path, content })`. Template processing + `readTextFile` for template files stay in TS (pure string transforms). Flag off preserves legacy behaviour. Tests pin both paths.
- [x] **8.8** Migrated `templates.service.ts::ensureTemplatesFolder`: when flag on, folder creation uses `invoke('create_folder', ...)` and empty template files use `invoke('create_note', { path, content: '' })`. Flag off preserves legacy behaviour.
- [x] **8.9 (partial)** Audited remaining `writeTextFile` / `mkdir` call sites. Split into two buckets:
  * **Config writes (NO migration needed)** — `auto-move`, `bookmarks`, `file-icons`, `todoist` write to `.kokobrain/*.json` internal app state, not vault notes. These legitimately don't go through the Rust VaultIndex. Documented as intentional.
  * **Vault content writes (follow-up needed)** — `tasks.service.ts:113` (task toggle), `deep-link.service.ts` (append / prepend actions), `canvas.service.ts` (canvas files). Each needs a `writeTextFile` + `invoke('update_note_in_index', { path, content })` pattern to keep the index in sync. Task toggle is the highest-priority as the tasks panel is a frequent interaction. Deferred to a dedicated follow-up PR to avoid scope creep here.
- [ ] **8.10** Properties Panel write branching (tab-open → `view.dispatch`; tab-closed → invoke). Deferred — current Properties Panel UI only edits the active tab, so the TS editor-buffer path is already correct. If a future feature adds bulk property edits on closed tabs, `update_frontmatter_v2` is ready.
- [ ] **8.11** Enable flags + delete orphans. Needs real-vault validation period first (see Phases 3.6-3.9 / 6.5 / 7.7 / 8.11 in the same category).
- [x] **8.12** Meta-bind `createNote` action — already covered transparently by Phase 8.7. The meta-bind button widget delegates to `openOrCreateNote` from `note-creator.service.ts`, which now invokes `create_note` when `rustProperties` is on. Meta-bind `updateMetadata` actions continue using `view.dispatch` (category b — live-preview widgets, CM is source of truth while tab is open) per ADR 0025. `parseFrontmatterProperties` cache by frontmatter substring is a separate perf optimisation deferred to a follow-up.
