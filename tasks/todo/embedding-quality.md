...
- [x] Task 2.4: Global `RERANKER: Mutex<Option<Reranker>>` mirroring `EMBEDDER`. Lazy load + idle unload (reuse 120s timeout). `ensure_reranker_loaded()` returns `Ok(false)` when model file isn't on disk so the search path can fall back gracefully. New commands `is_reranker_model_available` + `download_reranker_model`.
- [x] Task 2.5: `search_semantic` pipeline change — top-50 cosine candidates → rerank → adaptive filter on rerank logits → top-K. When the reranker model isn't downloaded yet, the pipeline transparently falls back to pure cosine ordering. Log line shows `reranker=true/false` so we can verify which path ran.
