...
- [ ] Task 1: Adicionar teste para finding #12 (chunks_exact silent truncation) em `src-tauri/tests/db_semantic_repo_test.rs`. DETERMINÍSTICO. Roda no CI sem `#[ignore]`.
- [ ] Task 2: Adicionar teste para finding #11 (update_entry retroactive race) em `src-tauri/tests/vault_index_test.rs`. NÃO-DETERMINÍSTICO. Marcado `#[ignore]`.
