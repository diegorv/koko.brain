...
describe('SETTINGS_SECTION_GROUPS', () => {
	...
	it('contains all expected sections in order', () => {
		...
		expect(ids).toEqual(['appearance', 'editor', 'sidebar', 'periodic-notes', 'quick-capture', 'one-on-one', 'templates', 'types', 'search', 'file-history', 'auto-move', 'trash', 'queryjs', 'todoist', 'troubleshooting', 'update']);
	});
