...
describe('SETTINGS_SECTION_GROUPS', () => {
	...
	it('contains all expected sections in order', () => {
		...
		expect(ids).toEqual(['appearance', 'editor', 'sidebar', 'periodic-notes', 'quick-note', 'one-on-one', 'templates', 'search', 'file-history', 'auto-move', 'trash', 'terminal', 'todoist', 'security', 'troubleshooting', 'update', 'experimental']);
	});
