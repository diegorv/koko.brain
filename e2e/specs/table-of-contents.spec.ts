...
test.describe('Table of Contents', () => {
	test('shows headings from the open file', async ({ vaultPage: page }) => {
		...
		// Welcome.md has "# Welcome"
		await expect(page.locator('button', { hasText: 'Welcome' })).toBeVisible({ timeout: 5_000 });
	});
	...
	test('updates when switching files', async ({ vaultPage: page }) => {
		...
		await expect(page.locator('button', { hasText: 'Tasks' })).toBeVisible({ timeout: 5_000 });
	});
